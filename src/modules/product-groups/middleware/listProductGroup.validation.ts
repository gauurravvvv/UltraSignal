/**
 * ListProductGroupValidation — validates pagination, sort, and filter.
 */
...
export const PRODUCT_GROUP_LIST_SORT_FIELDS = [
  'name',
  'status',
  'createdOn',
] as const;
export type ProductGroupListSortField =
  (typeof PRODUCT_GROUP_LIST_SORT_FIELDS)[number];

const ListProductGroupValidation = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  const schema = Joi.object({
    ...
    sourceId: Joi.alternatives()
      ...
      .optional(),
    sort: buildSortJoi(PRODUCT_GROUP_LIST_SORT_FIELDS).optional(),
  }).unknown(false);
