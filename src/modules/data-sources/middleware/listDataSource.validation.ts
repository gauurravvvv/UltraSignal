/**
 * ListDataSourceValidation — validates pagination + sort query params.
 * The filter object is a JSON-encoded blob the controller parses; this
 * validator only checks the wrapper shape (page, limit, sort field
 * whitelist).
 */
...
export type DataSourceListSortField = 'name' | 'status' | 'createdOn';

const ALLOWED_SORT_FIELDS: DataSourceListSortField[] = [
  'name',
  'status',
  'createdOn',
];
...
const schema = Joi.object({
  ...
  if (value.sort) {
    ...
    if (!ALLOWED_SORT_FIELDS.includes(field as DataSourceListSortField)) {
...
const ListDataSourceValidation = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export default ListDataSourceValidation;
